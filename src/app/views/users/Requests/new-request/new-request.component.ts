...
export class NewRequestComponent implements OnInit {
  ...
  @ViewChild('csvInput2') csvInput2!: ElementRef<HTMLInputElement>;
  siteslist: any[] = [];
  buildings: any[] = [];
  floors: any[] = [];
  ...
  isnewexcaworkyes: boolean = false;
  constructor(
  ...
  triggerFileInput1(): void {
      this.csvInput2?.nativeElement.click();
 }
  ...
  GetselectedNEExcaitem(event) {
    if (event == 1) {
      this.isnewexcaworkyes = true;
    } else {
      this.isnewexcaworkyes = false;
    }
  }
  ...
  CreateRequest() {
    ...
    this.Requestdata.picture = this.RequestForm.controls["picture"].value;
    this.Requestdata.location = this.RequestForm.controls["location"].value;
    // this.Requestdata.underground_areas_cleared = this.RequestForm.controls["floatLabel79"].value;
    ...
    // }
     for (const [key, value] of Object.entries(this.Requestdata)) {
      if (key !== 'rams_file' && key !== 'picture' && value !== null && value !== undefined) {
    ...
    if(this.Requestdata.picture) {
      (this.Requestdata.picture as File[]).forEach((file: File) => {
  ...
  : null;
      ...
      this.updaterequestdata.picture = this.RequestForm.controls["picture"].value;
      this.updaterequestdata.location = this.RequestForm.controls["location"].value;
...
for (const [key, value] of Object.entries(this.updaterequestdata)) {
      ...
      // formData.append("picture", JSON.stringify(this.updaterequestdata.picture))
      
      if(this.updaterequestdata.picture) {
      (this.updaterequestdata.picture as File[]).forEach((file: File) => {
        formData.append('picture', file);  
      });
    }
...
this.RequestForm.controls["Safetyprecaustion"].setValue(precautionIds);
    ...
    // this.RequestForm.controls["picture"].setValue(data["picture"]);
    // this.imagesExca = data.picture.map(file => ({
    //   name: file.picture, 
    //   ...file              
    // })); 
    this.imagesExca =[ { name: data["picture"] }];
    console.log(this.imagesExca['name'], 'img')
    this.RequestForm.controls["location"].setValue(data["location"]);
    this.isnewexcaworkyes = data["underground_areas_cleared"] == 1 ? true : false;
...
csvInputChange2(e: any): void {
  const files = e.target.files;
  this.imagesExca = []; // Clear previous selection

  if (files && files.length > 0) {
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      this.imagesExca.push(file);
    ...
    this.RequestForm.controls["picture"].setValue(this.imagesExca)
  }
}
